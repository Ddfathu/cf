...
run_tunnel_module() {
    while true; do
        clear
        echo -e "${CYAN}================================================================${NC}"
        echo -e "${BOLD}📌 SUB-MENU MANAJEMEN CLOUDFLARE ARGO TUNNEL:${NC}"
        echo -e "${CYAN}----------------------------------------------------------------${NC}"
        ...
        case "$TUN_CHOICE" in
            ...
            2)
                ...
                read -rp "Pilih Tunnel Target [0=Batal]: " T_NUM; [ "$T_NUM" == "0" ] && continue
                ...
                read -rp "Pilih Domain Utama [0=Batal]: " Z_NUM; [ "$Z_NUM" == "0" ] && continue
                SELECTED_DOMAIN=$(echo "$ZONES_JSON" | jq -r ".result[$((Z_NUM-1))].name")
                SELECTED_ZONE_ID=$(echo "$ZONES_JSON" | jq -r ".result[$((Z_NUM-1))].id")

                read -rp "Subdomain [Ketik '-' jika root, 0=Batal]: " SUB_INPUT; [ "$SUB_INPUT" == "0" ] && continue
                [ "$SUB_INPUT" == "-" ] && FULL_HOSTNAME="$SELECTED_DOMAIN" || FULL_HOSTNAME="${SUB_INPUT}.${SELECTED_DOMAIN}"

                read -rp "Port Lokal Application [0=Batal]: " PORT_INPUT; [ "$PORT_INPUT" == "0" ] && continue
                read -rp "Protokol [http/https/tcp/ssh] (Default: http): " SCHEME_INPUT; SCHEME_INPUT=${SCHEME_INPUT:-http}
                ...
                echo -e "${GREEN}✅ Route & DNS CNAME https://$FULL_HOSTNAME Berhasil Dibuat!${NC}"
                read -rp "Tekan Enter untuk kembali ke Sub-Menu..."
                ;;
            3)
                TUNNELS_JSON=$(curl -s "${AUTH_HEADER[@]}" -H "Content-Type: application/json" "$BASE_URL/accounts/$ACCOUNT_ID/cfd_tunnel?is_deleted=false")
                echo "$TUNNELS_JSON" | jq -r '.result[] | "\(.name) (ID: \(.id))"' | nl -w2 -s') '
                read -rp "Pilih Tunnel [0=Batal]: " T_NUM; [ "$T_NUM" == "0" ] && continue
                ...
                # TARIK & TAMPILKAN TOKEN TUNNEL
                TOKEN_RES=$(curl -s "${AUTH_HEADER[@]}" -H "Content-Type: application/json" "$BASE_URL/accounts/$ACCOUNT_ID/cfd_tunnel/$TUNNEL_ID/token")
                TUNNEL_TOKEN=$(echo "$TOKEN_RES" | jq -r '.result // empty')
                if [ -n "$TUNNEL_TOKEN" ]; then
                    echo -e "🔑 TOKEN: ${CYAN}$TUNNEL_TOKEN${NC}"
                fi
            ...
            4|5)
                ...
                read -rp "Pilih Routing Domain [0=Batal]: " R_NUM; [ "$R_NUM" == "0" ] && continue
                SELECTED_RULE=$(echo "$VALID_INGRESS" | jq -r ".[$((R_NUM-1))]")
                EDIT_HOSTNAME=$(echo "$SELECTED_RULE" | jq -r '.hostname')

                if [ "$TUN_CHOICE" == "4" ]; then
                    read -rp "Port Lokal Baru [0=Batal]: " NEW_PORT; [ "$NEW_PORT" == "0" ] && continue
