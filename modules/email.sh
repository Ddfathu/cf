...
check_and_fix_mx_records() {
    ...
    if [ "$MX_COUNT" -gt 0 ]; then
        ...
        curl -s -X POST "${AUTH_HEADER[@]}" -H "Content-Type: application/json" -d "{\"type\":\"MX\",\"name\":\"$domain_name\",\"content\":\"linda.mx.cloudflare.net\",\"priority\":62,\"ttl\":1}" "$BASE_URL/zones/$zone_id/dns_records" > /dev/null
...
manage_destination_addresses() {
    while true; do
        clear
        echo -e "${CYAN}================================================================${NC}"
        echo -e "${BOLD}📌 MANAJEMEN EMAIL PENERIMA (DESTINATION ADDRESS):${NC}"
        echo -e "${CYAN}----------------------------------------------------------------${NC}"
        ...
        echo -e "  ${RED}[0]${NC} ↩️  Kembali ke Menu Email Routing"
        echo -e "${CYAN}----------------------------------------------------------------${NC}"
        ...
        case "$DEST_CHOICE" in
            1)
                ...
                ;;
            2)
                read -rp "✉️  Masukkan Email Penerima Baru [0=Batal]: " NEW_DEST_EMAIL
...
run_email_routing_module() {
    while true; do
        clear
        echo -e "${CYAN}================================================================${NC}"
        echo -e "${BOLD}📌 SUB-MENU CLOUDFLARE EMAIL ROUTING:${NC}"
        echo -e "${CYAN}----------------------------------------------------------------${NC}"
